...
def fig2(N=None, s_stale=None, n_rec=None) -> str:
    ...
    out = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" '
           f'viewBox="0 0 {W} {H}"><rect width="{W}" height="{H}" fill="{SURFACE}"/>']
    out.append(text(40, 52, f"A {n_rec:,}-ID sample has a {detect(n_rec / N) * 100:.1f}% "
                            f"chance of finding {s_stale} stale documents",
                    23, INK, "600"))
    ...
    out.append(text(40, 560, "For rare defects a sample is a weak cutover gate, not a cheaper "
                             "approximation of exact verification:", 13.5, INK2))
    out.append(text(40, 582, f"once it has to cover {hi * 100:.0f}% of the corpus to be "
                             f"trustworthy, its only advantage is gone.", 13.5, INK2))
    out.append("</svg>")
    return "\n".join(out)
...
def fig3(batch=2000) -> str:
    ...
    # Each cell is represented by the run whose cutover-ready time is the
    # median, and that run's own phases are drawn. Summing per-phase medians
    # instead would produce bars that do not add up to any real run.
    reps = {}
    for cell, rs in cells.items():
        ordered = sorted(rs, key=lambda r: r["cutover_ready_seconds"])
        reps[cell] = ordered[len(ordered) // 2]
    ...
    W, H = 1180, 640
    PX0, PX1 = 300, 1020
    # The run's own recorded total, not the sum of its rounded phases, so the
    # label matches the results table exactly.
    totals = {c: reps[c]["cutover_ready_seconds"] for c in order}
    ...
    o.append(text(40, 78, f"Time to an index that has passed the cutover gate. Median run of "
                          f"{n_desc} per cell, "
    ...
    for cell, y in zip(order, ys):
        ...
        # Sum the paused phases directly. Subtracting the scan from the total
        # would fold in the live drain, which runs while writes continue.
        paused = sum(m(cell, f) for _, f, _, is_live in PHASES if not is_live)
        o.append(text(x + 12, y + 22, f"finalization {paused:.1f}s", 11.5, MUTED))
