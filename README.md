...
- Replicas and refresh are separated into a 2×2, each variable held constant
  ...
  throughout.
- The finalization window is 6.5–13.8s, and it is shard allocation plus
...
- The cutover gate requires exact comparison clean, shards green, zero
  unresolved writes, zero remaining lag, a drain that did not stall, and a
  canary write that landed in the target index. If any fails, the alias does
  not move. The alias swap itself is checked for acknowledgement.
