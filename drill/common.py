...
def point_alias(client: Elasticsearch, alias: str, to_index: str) -> float:
    """Atomically repoint an alias. Returns how long _aliases took.

    The actions apply atomically, but the response still carries an
    acknowledged flag; an unacknowledged swap has not taken effect
    cluster-wide.
    """
    ...
    resp = client.indices.update_aliases(actions=actions)
    elapsed = time.perf_counter() - t0
    if not resp.get("acknowledged", False):
        raise RuntimeError(f"alias swap to {to_index} was not acknowledged")
    return elapsed
